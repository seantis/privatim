...
def cleanup_duplicate_agenda_preferences(config_uri, dry_run):
    """
    ...
    with env['request'].tm:
        ...
        for user_id, agenda_item_id, count in duplicates:
            ...
            preferences = session.query(
                AgendaItemStatePreference.id).filter_by(
                user_id=user_id,
                agenda_item_id=agenda_item_id
            ).order_by(AgendaItemStatePreference.id).all()
        ...
        if dry_run:
            print('\n--- DRY RUN ---')
            print(
                f'{len(ids_to_delete)} duplicate entries would be deleted:'
            )
        ...
        session.execute(
            AgendaItemStatePreference.__table__.delete().where(
